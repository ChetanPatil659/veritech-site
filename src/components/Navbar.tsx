...
export default function Navbar({
  ...
  return (
    <div className="relative z-[99]">
      <nav
        className={`fixed top-0 left-0 flex items-center justify-between w-full px-10 py-2 ${
          expanded ? "bg-black" : `bg-[${bgColor}]`
        }`}
      >
        <NavLink to="/">
      ...
      <div
        className={`${
          expanded
            ? "h-screen w-screen rounded-none left-0 top-0"
            : "w-0 h-0 rounded-3xl -left-1/4 -top-1/4"
        } transition-transform duration-700 bg-black fixed z-[49] overflow-hidden`}
      >
        <ul
          className={`${
            expanded ? "opacity-100" : "opacity-0"
        ...
        >
          {[
            ...
            <li
              key={path}
              className="hover:text-white text-gray-600 transition-all duration-300 cursor-pointer"
            >
              {onClick ? (
